(() => {
  ...
  let mainElement = document.querySelector('.recipe');
  let procedureDialog = document.querySelector('.recipe-dialog-procedure');
  let notesDialog = document.querySelector('.recipe-dialog-notes');
  let notesList = document.querySelectorAll('.recipe-note');
    ...
    function nothingToSeeHere () {
      let emptyStatement = `<p class="recipe-dialog-notes-nothing">There are no notes with this recipe.</p>`;
      notesDialog.insertAdjacentHTML('beforeend', emptyStatement);
    };

  mainElement.addEventListener('click', (event) => {
    let whichButton = event.target.dataset.source;
